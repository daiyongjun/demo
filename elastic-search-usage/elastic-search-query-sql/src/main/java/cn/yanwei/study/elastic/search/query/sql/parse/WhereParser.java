...
import cn.yanwei.study.elastic.search.query.sql.SQLFunctions;
...
import cn.yanwei.study.elastic.search.query.sql.spatial.SpatialParamsFactory;
...
class WhereParser {
    ...
    WhereParser(SqlParser sqlParser, MySqlSelectQueryBlock query) {
    ...
    WhereParser(SqlParser sqlParser, SQLExpr expr) {
        this.sqlParser = sqlParser;
        this.where = expr;
    }

    WhereParser(SqlParser sqlParser) {
        this.sqlParser = sqlParser;
    }

    Where findWhere() throws SqlParseException {
        if (where == null) {
            return null;
        }
    ...
    void parseWhere(SQLExpr expr, Where where) throws SqlParseException {
    ...
    private boolean explainSpecialCondWithBothSidesAreLiterals(SQLBinaryOpExpr bExpr, Where where) throws SqlParseException {
        boolean condition = (bExpr.getLeft() instanceof SQLNumericLiteralExpr || bExpr.getLeft() instanceof SQLCharExpr);
        boolean condition1 = (bExpr.getRight() instanceof SQLNumericLiteralExpr || bExpr.getRight() instanceof SQLCharExpr);
    ...
    private boolean explainSpecialCondWithBothSidesAreProperty(SQLBinaryOpExpr bExpr, Where where) throws SqlParseException {
        //join is not support
        boolean condition = (bExpr.getLeft() instanceof SQLPropertyExpr || bExpr.getLeft() instanceof SQLIdentifierExpr);
        boolean condition1 = (bExpr.getRight() instanceof SQLPropertyExpr || bExpr.getRight() instanceof SQLIdentifierExpr);
        boolean condition2 = Sets.newHashSet("=", "<", ">", ">=", "<=").contains(bExpr.getOperator().getName());
        boolean condition3 = !Util.isFromJoinOrUnionTable(bExpr);
        if (condition && condition1
                && condition2
                && condition3
        ) {
            ...
            String regex = "\\.";
            if (leftProperty.split(regex).length > 1) {

                leftProperty = leftProperty.substring(leftProperty.split(regex)[0].length() + 1);
            }

            if (rightProperty.split(regex).length > 1) {
                rightProperty = rightProperty.substring(rightProperty.split(regex)[0].length() + 1);
            }
    ...
    private boolean isAllowedMethodOnConditionLeft(SQLMethodInvokeExpr method, SQLBinaryOperator operator) {
        return ("nested".equals(method.getMethodName().toLowerCase()) ||
                "children".equals(method.getMethodName().toLowerCase()) ||
    ...
    private void explanCond(String opear, SQLExpr expr, Where where) throws SqlParseException {
        if (expr instanceof SQLBinaryOpExpr) {
            ...
            if (SpatialParamsFactory.isAllowedMethod(methodName)) {
                ...
                where.addWhere(condition);
            } else if ("children".equals(methodName.toLowerCase())) {
                ChildrenType childrenType = new ChildrenType();
