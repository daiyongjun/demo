...
public class ElasticSqlExprParser extends SQLExprParser {
    ...
    private SQLExpr primary2() {
        ...
        switch (tok) {
            ...
            case VARIANT:
                ...
                lexer.nextToken();
                if ("@@global".equalsIgnoreCase(varRefExpr.getName())) {
                    accept(Token.DOT);
                    varRefExpr = new SQLVariantRefExpr(lexer.stringVal(), true);
                    lexer.nextToken();
                } else if ("@".equals(varRefExpr.getName()) && lexer.token() == Token.LITERAL_CHARS) {
    ...
    public SQLColumnDefinition parseColumnRest(SQLColumnDefinition column) {
        ...
        if (identifierEquals("precision") && "double".equalsIgnoreCase(column.getDataType().getName())) {
            lexer.nextToken();
        }
    ...
    MySqlSelectGroupByExpr parseSelectGroupByItem() {
