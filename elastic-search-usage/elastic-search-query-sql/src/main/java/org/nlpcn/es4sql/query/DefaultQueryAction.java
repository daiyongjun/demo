...
public class DefaultQueryAction extends QueryAction {
	...
	public void intialize(SearchSourceBuilder request) throws SqlParseException {
		this.searchSourceBuilder = request;
	}
...
//			searchSourceBuilder.setScroll(new TimeValue(timeoutInMilli)).setSize(scrollSize);
		}
	...
	public void setFields(List<Field> fields) throws SqlParseException {
		if (select.getFields().size() > 0) {
			...
			searchSourceBuilder.fetchSource(includeFields.toArray(new String[includeFields.size()]), excludeFields.toArray(new String[excludeFields.size()]));
		}
	}
